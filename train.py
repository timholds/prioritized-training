...
def main():
    ...
    big_batch_size = int(batch_size / args.subsample_rate)
    steps_per_epoch = len(x_train) // big_batch_size
    holdout_epochs = config['holdout_epochs']
    epochs = int(config['epochs'] / args.subsample_rate)
    ...
    # Get or train holdout model (shared across all seeds)
    print(f"\n{'='*50}")
    ...
    # Compute IL losses using holdout model (shared across all seeds)
    print(f"\nComputing IL losses...")
    ...
    result_file = f"results/{args.dataset}_subsample{args.subsample_rate}.json"
